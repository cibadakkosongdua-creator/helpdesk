...
import { ClipboardList, MessageCircle, Send, Sparkles, ShieldCheck, User, Check, CheckCheck } from "lucide-react"
            ...
            >
              ...
              <div className={`max-w-[80%] flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                ...
                <div className="flex items-center gap-1.5 text-[10px] font-semibold text-slate-400 dark:text-slate-500 mt-1">
                  ...
                  {/* Read receipt - only show for my messages */}
                  {isMe && (
      ...
      {isResolved ? (
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100"
          >
            {sending ? (
              <span className="w-3.5 h-3.5 border-2 border-slate-300 border-t-white dark:border-t-slate-900 rounded-full animate-spin" />
            ) : (
